...
if __name__ == '__main__':
    ...
    for text in classify_texts:
        ...
        if category == 'spam':
            ...
            spam_names.append(file_names[total_count])
            spam_probs.append(probability)
    ...
    for string in train_words:
        string = string.strip(',')
        string = string.strip('.')
        train_words_lower.append(string.lower())

    train_words.extend(train_words_lower)
    
    threshold = float(len(train_words)/2) * .8
